...
void printmatrix(char *s, float *mat, int m, int n);
...
debug_calcndc(float *m) {
	...
	printmatrix("debug_ndc_coords", ndc_coords, 3, 4);
}

void
printmatrix(char *s, float *mat, int m, int n) {
	int i, j;
	printf("%s:\n", s);
	for (i = 0; i < m; i++) {
		for (j = 0; j < n; j++) {
...
/* 'fov' is in degrees */
void
projectmat(float *m, float fov, float r, float near, float far) {
